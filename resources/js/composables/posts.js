...
export default function usePosts(){
   ...
   const getPosts = async(page=1,search_category='',search_id='',search_title='',search_content='',search_global='',order_column='created_at',order_direction='desc') =>
   {
      axios.get(
         '/api/posts?page='+page+
   ...
   {      
      ...
      axios.post('/api/posts',serializedPost)
         .then(response => {
            ...
            swal({icon:'success',title:'Post saved successfully'}) 
         })
         .catch(error => { 
            ...
            if(result.isConfirmed){ 
               axios.delete('/api/posts/'+id)
                  .then(response => {
                     getPosts()
                     router.push({name:'posts.index'})
                     swal({icon:'success',title:'Post deleted successfully'})
                  })
                  .catch(error => {
